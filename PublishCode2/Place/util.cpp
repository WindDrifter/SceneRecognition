...
double KNN_Vote::GetBestScore() 
{ 
    return scores[0]; 
}

int KNN_Vote::GetBestScoreClass() 
{
    return votes[0]; 
}

int KNN_Vote::GetBestScoreIndex() 
{
    return indexes[0]; 
}

int KNN_Vote::GetLabel(const int pos)
{
    assert(pos>=0 && pos<K);
    return votes[pos];
}

int KNN_Vote::GetIndex(const int pos)
{
    assert(pos>=0 && pos<K);
    return indexes[pos];
}

double KNN_Vote::GetScore(const int pos)
{
    assert(pos>=0 && pos<K);
    return scores[pos];
}

int KNN_Vote::GetVotedClass(const int nclass) // assume all class labels (votes[i]) are integers between {0,1,2,..,nclass-1}
{
...
int KNN_Vote::GetWeightVotedClass(const int nclass)
{
